...
WITH customer_orders AS (
    SELECT
        ...
        ol.qty,
        ol.net_line_amount
    FROM `gulf-retail-30days.gulf_retail.customers` AS c
    INNER JOIN
        `gulf-retail-30days.gulf_retail.orders` AS o
...
customer_metrics AS (
    SELECT
        ...
        SAFE_DIVIDE(SUM(net_line_amount), COUNT(DISTINCT order_id))
            AS avg_order_value
...
SELECT
    *,
    ...
    CASE
        WHEN
            DATE_DIFF(CURRENT_DATE(), last_order_date, DAY) <= 30
            AND lifetime_net_revenue >= 2000
            THEN 'Active High-Value'
        WHEN
            DATE_DIFF(CURRENT_DATE(), last_order_date, DAY) <= 30
            THEN 'Active Regular'
        WHEN
            DATE_DIFF(CURRENT_DATE(), last_order_date, DAY) <= 90
            THEN 'At Risk'
        ELSE 'Dormant'
